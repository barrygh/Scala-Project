...
class KNNModel(private var groupOne: ArrayList[KNNData], private var groupTwo: ArrayList[KNNData]) {
    ...
    def majorityGroup(sortedPairList: ArrayList[Pair[Double, KNNData]]): Int = {
        ...
        while (i < sortedPairList.size) {
            ...
            if (i + 1 < sortedPairList.size) {
                ...
                if (sortedPairList.get(i).getFirst == distNext && sortedPairList.get(i).getSecond.getClassNum != classnumNext) {
                    // ...skip forward.
                    k = k + 2
                    i = i + 1
                    ...
                    k = k + 1
                    if (sortedPairList.get(i).getSecond.getClassNum == 1) {
                        g1 += 1
                        g1 - 1
                    } else {
                        g2 += 1
    ...
    def printKNNList(list: ArrayList[KNNData]): Unit = {
        for (i <- 0 until list.size) {
            ...
            println("i: %d | KNN (%d, %d, %d)".format(i, p1, p2, cn))
        }
        println()
    }

    def printPairList(pairList: ArrayList[Pair[Double, KNNData]]): Unit = {
        println("list size: %d".format(pairList.size))
        for (i <- 0 until pairList.size) {
            ...
            println("i: %d | distance: %f | KNNData: (%d,%d,%d)".format(i, distance, p1, p2, cN))
        }
        println("_______")
    }
}
